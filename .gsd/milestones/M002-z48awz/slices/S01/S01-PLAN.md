...
## Tasks
- [ ] **T01: Extend ArtifactMetadata schema and add GQL metadata fetch** — Add three new display fields to ArtifactMetadata and a new GQL function in twitch.rs. Purely library code — no call-site changes in main.rs or cli.rs. T02 wires the results into the runtime paths.
  - Estimate: 45m
  - Files: src/artifact.rs, src/twitch.rs
  - Verify: cargo test 2>&1 | grep -E 'test result|FAILED|^error'
- [ ] **T02: Wire vod context into download paths and add --skip-metadata flag** — T01 produced the schema changes and fetch_vod_metadata_by_id. This task wires them into the two download call paths and adds the --skip-metadata CLI flag.
  - Estimate: 45m
